...
## Architecture
- Microservices: `auth` (3000), `clubs` (3001), `events` (3002), `notifications` (3004), `users` (3005). Each service exposes REST endpoints under its own prefix (e.g., `/clubs`, `/users`).
...
To avoid CORS issues, serve it over HTTP:

```powershell
npm install -g http-server
http-server "<path-to-frontend-folder>" -p 5500
...
Note: If you prefer simpler URLs (e.g., `GET /clubs` at the gateway → `GET /clubs` at service), change the gateway `pathRewrite` rules to preserve prefixes and then use single-prefix paths in the frontend and tests.
...
- With current mount behavior, tests target doubled paths (e.g., `/clubs/clubs`) because Express removes the mount `/clubs` before proxying. If you change `pathRewrite` to preserve the prefix, tests should target single-prefix paths (e.g., `/clubs`).

## Troubleshooting
- Port conflicts: If a service fails with `EADDRINUSE`, free the port or change the service port and update the gateway mapping accordingly.
- CORS: If the browser blocks requests, serve the frontend via `http-server` and ensure `app.enableCors({ origin: true })` is set in `gateway/src/main.ts`.
- Users service: If `users` fails to start, e2e tests that call users endpoints will fail. Start or fix `users` first.
